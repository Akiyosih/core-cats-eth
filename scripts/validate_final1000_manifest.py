...
VALID_TIERS = {"base", "rare", "superrare"}
VALID_RARE_TYPES = {"odd_eyes", "red_nose", "blue_nose", "glasses", "sunglasses"}
VALID_SUPERRARE_TYPES = {"corelogo_1", "corelogo_2"}
...
def main() -> int:
    ...
    for idx, it in enumerate(items):
        ...
        if tier == "base" and rtype != "none":
            add_error(errors, f"token {tid}: base token must have rarity_type=none (got {rtype})")
    ...
    if by_tier.get("base", 0) != 900:
        add_error(errors, f"Expected base=900, got {by_tier.get('base', 0)}")
